...
- Telegram CI final failure bloki takroriy `Runner/Test/Step` qatorlari o'rniga `Test`, `Qadam`, `Muammo`, `Texnik`, `Kod`, `Ta'sir`, `Yechim` tartibida chiqadi. Playwright call logdagi `locator resolved to` + `element is not visible` holati "element topilmadi" emas, "element topildi, ammo yashirin" deb ko'rsatiladi.
