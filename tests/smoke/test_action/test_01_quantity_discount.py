"""10 dona mahsulot xaridiga 10% chegirma beruvchi aksiya yaratish."""
...
pytestmark = [allure.epic("Smoke"), allure.feature("Aksiya Setup"), allure.story("01 - 10 dona mahsulotga 10% chegirma aksiyasi")]

# ----------------------------------------------------------------------------------------------------------------------


def run_quantity_discount(page, code):
    """10 dona mahsulot xaridiga 10% chegirma beruvchi aksiya yaratish.
    ...
    4. Обычный shart: setup mahsuloti uchun 10 chegarasini belgilash.
    5. Скидка bonusi: setup mahsulotiga 10% berish.
    ...
    """
    base = BasePage(page)
    action_name = f"action-quantity-discount-pw{code}"
    action_code = f"c_aqd_pw{code}"
    ...
    with allure.step("4 - Обычный shart va 10 chegarasini belgilash"):
    ...
    with allure.step("5 - 10% chegirmani belgilash"):
        ...
        base.input(ng_model="product.value", value="10", press_tab=True)
        base.ui_select(label="Тип условия", expect_value="Обычный")
    ...
    with allure.step("8 - Aksiya setup ma'lumotlarini saqlash"):
        ...
        save_data("action_quantity_discount", action_data)

# ----------------------------------------------------------------------------------------------------------------------


@allure.title("01 - 10 dona mahsulotga 10% chegirma aksiyasi")
def test_quantity_discount(page, code):
    with allure.step("User profil bilan setup filialiga kirish"):
        authorization(page, who="user", code=code)
        BasePage(page).switch_filial(name=f"filial-pw{code}")
    run_quantity_discount(page, code)
