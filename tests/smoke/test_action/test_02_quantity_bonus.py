"""10 dona mahsulot xaridiga shu mahsulotdan 1 dona bonus beruvchi aksiya yaratish."""
...
pytestmark = [allure.epic("Smoke"), allure.feature("Aksiya Setup"), allure.story("02 - 10 dona mahsulotga 1 dona bonus aksiyasi")]

# ----------------------------------------------------------------------------------------------------------------------


def run_quantity_bonus(page, code):
    """10 dona mahsulot xaridiga shu mahsulotdan 1 dona bonus beruvchi aksiya yaratish.
    ...
    """
    base = BasePage(page)
    action_name = f"action-quantity-bonus-pw{code}"
    action_code = f"c_aqb_pw{code}"
    ...
    with allure.step("8 - Aksiya setup ma'lumotlarini saqlash"):
        ...
        save_data("action_quantity_bonus", action_data)

# ----------------------------------------------------------------------------------------------------------------------


@allure.title("02 - 10 dona mahsulotga 1 dona bonus aksiyasi")
def test_quantity_bonus(page, code):
    with allure.step("User profil bilan setup filialiga kirish"):
        authorization(page, who="user", code=code)
        BasePage(page).switch_filial(name=f"filial-pw{code}")
    run_quantity_bonus(page, code)
