"""100 000 xarid summasiga shu mahsulotdan 1 dona bonus beruvchi aksiya yaratish."""
...
pytestmark = [allure.epic("Smoke"), allure.feature("Aksiya Setup"), allure.story("04 - 100 000 summaga 1 dona bonus aksiyasi")]

# ----------------------------------------------------------------------------------------------------------------------


def run_amount_bonus(page, code):
    """100 000 xarid summasiga shu mahsulotdan 1 dona bonus beruvchi aksiya yaratish.
    ...
    4. Обычный shart: setup mahsuloti uchun 100000 chegarasini belgilash.
    ...
    """
    base = BasePage(page)
    action_name = f"action-amount-bonus-pw{code}"
    action_code = f"c_aab_pw{code}"
    ...
    with allure.step("2 - Aksiyaning asosiy ma'lumotlarini to'ldirish"):
        ...
        base.ui_select(label="Тип акции", value="Сумма")
    ...
    with allure.step("4 - Обычный shart va 100000 chegarasini belgilash"):
        base.checkbox(label="Ассортимент", expect_checked=False)
        base.ui_select(label="Тип условия", value="Обычный")
        ...
        base.input(ng_model="rule.main_value", value="100000", press_tab=True)
        base.input(ng_model="rule.extra_value", expect_value="")
        base.input(ng_model="rule.required_count", value="1", press_tab=True)

    with allure.step("5 - 1 dona bonusni belgilash"):
        base.ui_select(label="Тип бонуса", value="Кол-во")
        ...
        base.ui_select(label="Тип условия", expect_value="Обычный")
        base.input(ng_model="rule.main_value", expect_value=re.compile(r"100\s*000"))
    ...
    with allure.step("7 - ID ustuni va saqlangan aksiya qiymatlarini tekshirish"):
        ...
        action_row = base.grid(action_name, "Сумма", start_date, end_date, "Активный")
    ...
    with allure.step("8 - Aksiya setup ma'lumotlarini saqlash"):
        ...
        save_data("action_amount_bonus", action_data)

# ----------------------------------------------------------------------------------------------------------------------


@allure.title("04 - 100 000 summaga 1 dona bonus aksiyasi")
def test_amount_bonus(page, code):
    with allure.step("User profil bilan setup filialiga kirish"):
        authorization(page, who="user", code=code)
        BasePage(page).switch_filial(name=f"filial-pw{code}")
    run_amount_bonus(page, code)
